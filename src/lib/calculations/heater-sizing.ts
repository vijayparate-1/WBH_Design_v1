...
export interface Stage3Results {
  ...
  // Fixed-leg shortfall detection
  nodalFinalT?: number;           // actual terminal gas temperature from nodal integration
  nodalThermalShortfall?: number; // °C below target when lenFixed && area_adequate === false
  nodalShortfallFlag?: boolean;   // true when shortfall > 0.5°C
}
...
export function calcStage3(inputs: Stage3Inputs): Stage3Results {
  ...
  // ── Nodal shortfall detection ──────────────────────────────────────────────
  // After the loop, current_T_gas is the temperature actually achieved.
  // If lenFixed && area_adequate === false, the coil cannot reach T_out_C.
  // We expose the shortfall so the UI can warn the user clearly.
  const nodalFinalT = Math.round(current_T_gas * 10) / 10;
  const nodalThermalShortfall = lenFixed && !area_adequate
    ? Math.max(0, T_out_C - nodalFinalT)   // °C short of target
    : 0;
  const nodalShortfallFlag = nodalThermalShortfall > 0.5;  // meaningful shortfall threshold

  // If shortfall exists, append a warning node so the profile clearly shows truncation
  if (nodalShortfallFlag) {
    nodalProfile.push({
      x: Math.round((current_x + dL) * 100) / 100,
      T_g_in: nodalFinalT,         // what was actually achieved
      Tb: T_bath_C,
      T_wall: nodalFinalT + 2,     // approximate — wall barely above gas at shortfall
      dQ: 0,                        // sentinel: no more heat transfer available
    });
  }
